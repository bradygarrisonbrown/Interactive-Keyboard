...
class TypingGame:
    def __init__(self):
        ...
        # Graphics
        # -------------------------
        self.bg = pygame.image.load("Images/sunset.jpg").convert()
    ...
    def handle_key(self, key):
        ...
        if key == expected:
            #Sheeple update
            self.sheeple_vy = -3  # bounce up
            self.sheeple_spin_speed = 0  # stop spinning
            ...
            #Sheeple update
            self.sheeple_spin_speed = 20  # spin fast
    ...
    def draw_scrolling_text(self):
        ...
        if current_pos:
            ...
            # hover slightly above letter
            target_y -= 20

            # move toward x smoothly
    ...
    def run(self):
        while self.running:
            ...
            if self.state == "title":
                ...
                # stop tiny jitter spin
                if abs(self.sheeple_spin_speed) < 0.1:
                    self.sheeple_spin_speed = 0

                # keep angle bounded (prevents huge numbers)
                self.sheeple_angle %= 360

                transformed = pygame.transform.smoothscale(self.sheeple_img, (120, 120)   # adjust size here
                                     )

                rotated = pygame.transform.rotate(transformed, self.sheeple_angle)
                
                rect = rotated.get_rect(center=(self.sheeple_x, self.sheeple_y))
